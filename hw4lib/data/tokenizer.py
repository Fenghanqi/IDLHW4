...
class H4Tokenizer:
    ...
    def get_avg_chars_per_token(self, token_ids: Union[List[int], torch.Tensor], skip_special_tokens: bool = True) -> float:
        """
        Calculate average number of characters per token for given token IDs.

        Args:
            token_ids: List of token IDs or Tensor to analyze
            skip_special_tokens: Whether to exclude special tokens from calculation

        Returns:
            Average number of characters per token
        """
        # Handle tensor input by converting to list if needed
        if isinstance(token_ids, torch.Tensor):
            token_ids_list = token_ids.tolist()
        else:
            token_ids_list = token_ids
            
        decoded_text = self.decode(token_ids_list, skip_special_tokens=skip_special_tokens)
        
        if skip_special_tokens:
            special_tokens = [self.pad_id, self.unk_id, self.mask_id, 
                              self.sos_id, self.eos_id, self.blank_id]
            token_count = sum(1 for id in token_ids_list if id not in special_tokens)
        else:
